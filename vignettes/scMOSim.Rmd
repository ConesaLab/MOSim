...
regulatory omic (scATAC-seq or transcription factors), scMOSim follows a
similar approach to the original MOSim approach. It simulates gene
coexpression in clusters and defines regulatory effects of the regulator
omic on the scRNA-seq (activator, repressor or no effect).
...
-   `data` if passed, is a user input list of matrices with features as
...
the code above, scMOSim simulates scRNAseq and scATACseq count matrices,
resulting in matrices with the following characteristics:
...
-   `diffGenes` OPTIONAL. If number groups \> 1, DE genes to simulate in
    the groups compared to group 1. Can be a vector with absolute genes
    for Up and Down ex: c(250, 500) or a percentage for up, down. The
    rest will be NE.
...
biological replicates and 2 groups, with 20% DE genes between groups,
8000 co-expressed genes split into 8 cluster patterns and a defined mean
and standard deviation expression per celltype, the call to scMOSim
would be as follows:
...
#               numberGroups = 2, diffGenes = 0.2, feature_no = 8000, clusters = 8, 
#               mean = c(2*10^6, 1*10^6,2*10^6, 1*10^6), sd = c(5*10^5, 2*10^5, 5*10^5, 2*10^5))
sim <- scMOSim(omic_list, cell_types, numberCells = c(10, 20, 30, 40), numberReps = 2, 
               feature_no = 8000, clusters = 8, 
               mean = c(2*10^6, 1*10^6,2*10^6, 1*10^6), sd = c(5*10^5, 2*10^5, 5*10^5, 2*10^5))
```
...
MOSim can also define the regulatory function (activator, repressor, or
no effect) of features in the regulatory omics data, i.e. the peaks in
scATAC-seq, without providing the optional arguments. It's mandatory
...
## 6.1 The *simulation* object

## 6.2 Retrieving the simulation settings: `sc_omicSettings`

## 6.3 Accessing the count data matrices: `sc_omicResults`

## 6.4 Obtaining the experimental design matrix: `sc_experimentalDesign`

# 7. Example full run

Let's visualize first the elbow plot for scRNA-seq and then use UMAP, a
non linear dimensional reduction, to place similar cells together in
...
Monzó C., Martínez-Mira C., Febbo A., Arzalluz-Luque A., Conesa A.,
