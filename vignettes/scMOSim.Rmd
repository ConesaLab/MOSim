...
regulatory omic (scATAC-seq or transcription factors), `scMOSim` follows
a similar approach to the original `MOSim` approach. It simulates gene
...
association_list <- data.frame("Peak_ID" = head(rownames(omicsList[[2]])), "Gene_ID" = head(rownames(omicsList[[1]])))


testing_groupsreps <- MOSim::scMOSim(omicsList, cell_types, numberReps = 2, numberGroups = 3, diffGenes = list(c(0.5, 0.4), c(0.3, 0.3)), minFC = 0.25, maxFC = 4, numberCells = NULL, mean = NULL, sd = NULL, regulatorEffect = list(c(10, 20), c(20, 10)), associationList = association_list)
```
